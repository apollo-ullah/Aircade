    ...
    private func playerScores(_ player: BadgePlayer) -> some View {
        VStack(alignment: .leading, spacing: 11) {
            HStack(spacing: 8) {
                ...
                Spacer()
                Button("Log out") { players.logout() }.font(.caption)
            }
            ...
            HStack {
                ...
                Spacer()
                Button { NSWorkspace.shared.open(players.leaderboardURL) } label: { Image(systemName: "trophy") }
                    .buttonStyle(.plain).help("Open leaderboard")
            }
