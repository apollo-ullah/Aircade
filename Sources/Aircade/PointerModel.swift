...
final class PointerModel: ObservableObject {
    ...
    private var mouseOverrideUntil = -Double.infinity
    ...
    func ingestMotion(orientation: simd_quatf, sampleAge: Double, speed: Double, time: Double) -> Bool {
        ...
        guard time >= mouseOverrideUntil else { return false }
        if wasAbsent { reacquiredAt = time; swing = SwingDetector() }
        source = .motion
    ...
    /// Deliberate mouse movement gives the pointer a short, dependable fallback.
    func ingestMouse(_ unit: CGPoint, time: Double = ProcessInfo.processInfo.systemUptime) {
        source = .mouse
        mouseOverrideUntil = time + 2
