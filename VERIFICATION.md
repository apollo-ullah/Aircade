...
The app requests return plans asynchronously from the authenticated local station and keeps rendering with a bounded local fallback. The station supports Baseten's OpenAI-compatible Model API, validates a complete ranking of supplied candidate IDs, rejects invented/invalid plans, and can also use the included Truss logistic model. Secrets load only from ignored `.env` state. After configuration, a live candidate-ranking request returned **Baseten Model API · zai-org/GLM-5.3-Flash · 951 ms** with `fallbackUsed: false`. The shared endpoint previously exceeded a 2.8-second limit, so the background-only server/app request windows are now 8/9 seconds; rendering never waits for them and late plans apply to a later ball.
