...
test('Baseten LLM ranks only legal returns and reports remote inference', async () => {
  ...
  const child = spawn(process.execPath, ['server.mjs'], {
    cwd: import.meta.dirname,
    env: {
      ...process.env, PORT: '8789', MONGODB_DB: database,
  ...
  try {
    ...
    for (let i = 0; i < 100; i++) {
      if (child.exitCode !== null) throw new Error(output);
      try { if ((await fetch('http://127.0.0.1:8789/api/health')).ok) { ready = true; break; } } catch {}
      await new Promise(resolve => setTimeout(resolve, 100));
    }
    assert.ok(ready, output);
    const config = JSON.parse(readFileSync(new URL('../.local/station.json', import.meta.url)));
    ...
    const response = await fetch('http://127.0.0.1:8789/api/tennis/opponent-plan', {
      method: 'POST',
