...
| Opponent | Animated Tennis rival with asynchronous Baseten candidate ranking, validated tactics and deterministic local fallback | Complete a physical match while the live provider is enabled; do not describe the bounded policy as end-to-end reinforcement learning |

181 Swift tests pass, including generated QR images through the production Vision reader, controller/session integration, Tennis opponent behavior and collision recovery. The Node station suite covers MongoDB profiles and leaderboards plus mocked Baseten authorization, response validation and fallback behavior. A live candidate-ranking request completed through Baseten Model API using `zai-org/GLM-5.3-Flash` in 951 ms without fallback. These checks establish software and service behavior, not motion latency or physical usability.
...
4. **Make the existing opponent legible.** Tennis already requests bounded tactics from Baseten between points, validates the selected candidate and keeps physics plus a deterministic fallback local. Show the provider status and one visible behavior change in a short match. Do not call this end-to-end model training or imply that network inference runs in the frame loop.
...
Sponsor choices must match the team's submitted selections. **Baseten is the strongest current fit:** the station makes authenticated Model API requests to rank constrained Tennis tactics, validates the response, and falls back locally without interrupting play. OpenAI still requires an OpenAI API-powered product experience in addition to meaningful Codex development; Codex usage alone does not satisfy that product requirement. Sentry asks for at least two products beyond error monitoring and evidence that they improved the project. QR badge sign-in should not be presented as deep badge-hardware work. [Official prize requirements](https://hackthenorth2026.devpost.com/)

## Honest conclusion

Aircade now has a coherent, technically substantial arcade loop and a real hosted-opponent integration. Its main risks are perceived derivative gameplay, AirPod lag, physical handoff friction and unverified new-build hardware behavior. Reliable hands-on surprise and demonstrable depth are the next milestones. More channels or sponsor logos will not substitute for them.
