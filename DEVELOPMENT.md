...
Swift owns animation and physics, including court bounces and opponent reach. The native app asks the local station for a validated shot decision asynchronously. **Astra · OpenAI API** and **Jev** share the same state, five legal shots, objective, and six-second API deadline. A physical tactical match requires a valid model plan before starting. Later refresh failures retain the previous genuine decision with a **Cached** label; responses from paused or abandoned runs cannot replace the current plan.

Put `OPENAI_API_KEY` and `AI_GATEWAY_API_KEY` in the ignored repository-root `.env` file for Astra and Jev respectively. API keys remain on the local station. Start it with `./scripts/start-station.sh` in one terminal, then run `./scripts/run.sh` in another. The launcher stages the local station connection in this checkout's private Application Support directory, outside the app bundle. Configuration reads run off the UI thread and time out if access stalls. Run `node scripts/prepare-launch.mjs` again after changing station settings. The station preserves the configured port; this judging checkout currently uses 8794.

During tennis, expand **AI shot decisions** to see the input, latest response, and the decision actually applied on court. It links to the bundled recorded comparison and concrete Codex debugging story. **How it works** opens the sensor pipeline with live readings and recorded playback. The bundled eighteen-second sample contains real AirPod readings with an explicitly reconstructed racket mapping. New captures preserve the current mapping and available game events; replay never controls or scores the game.

See [the judging demo and submission copy](docs/JUDGING-SUBMISSION-DRAFT.md), [the current submission audit](docs/SUBMISSION-AUDIT.md), and [the reproducible comparison runner](server/benchmark/README.md). The recorded comparison measures legal decisions and request time, not tennis ability. Desktop computer use is a separate slowed, buffered, unranked experiment.
